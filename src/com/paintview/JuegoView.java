...
//		}
		...
		private final String TAG = JuegoView.class.getSimpleName();
		...
		public static final int GAP_PROCESS = 20;
				...
				try {
					...
					synchronized (mSurfaceHolder) {
						...
						while (sleepTime < 0 && framesSkipped < MAX_FRAME_SKIPS) {
							// we need to catch up
							process.process(FRAME_PERIOD); // update without rendering
...
//						}
		...
		private void storeStats() {
			...
			if (statusIntervalTimer >= lastStatusStore + STAT_INTERVAL) {
				...
				process.setAvgFps("FPS: " + df.format(averageFps));
			}
		}
		...
		private void doDraw(Canvas canvas) {
			if (process != null && canvas != null)
				process.doDraw(canvas);
	...
	public void surfaceDestroyed(SurfaceHolder holder) {
		...
		// // Indicamos al thread que se cierre, y esperamos hasta que se
		// termine,
